...
## الميزات المتقدمة (v4) — ديناميكية بالكامل من Supabase

### ⚽ دوري الترجيح الحي (Live Penalty Shootout)
- **تحديات 1v1 في الزمن الحقيقي** عبر `penalty_rooms` + Supabase Realtime (اشتراك مباشر بتحديثات الغرفة)
- **Matchmaking تلقائي**: دالة `join_penalty_queue` تربط اللاعبين المنتظرين بـ `skip locked` (بلا ازدواجية)
- **لعب سريع ضد حاسوب TiQ**: دالة `start_bot_match` — الحاسوب يسدد باحتمالية حسب الصعوبة (سهل 75% / متوسط 55% / صعب 40%)
- **مؤقت تسديد 5 ثوانٍ** — المصدر الوحيد للزمن هو `deadline` من السيرفر (لا ثقة بالساعة المحلية)
- كل الأسئلة من جدول `penalty_questions` (ثنائية اللغة) — أضف أسئلة من لوحة Supabase دون تحديث التطبيق
- النتائج والتتويجات تُحسب في `penalty_stats` على السيرفر داخل الدوال (لا يمكن التلاعب بها)
- لوحة **ترتيب الرماة** مباشرة من view `penalty_leaderboard`

### 🎨 التخصيص الديناميكي للملف
- كتالوج أفاتارات `avatar_catalog` وكتالوج تاغات `tag_catalog` — يُدار بالكامل من لوحة Supabase
- **قفل حسب XP**: العناصر تُفتح تلقائيًا عند بلوغ `min_xp`
- الاختيارات تُحفظ في `profiles.avatar_id` و `profiles.tag_id`
- بطاقة **نسبة الانتصار في الترجيح** في إحصائيات البروفايل

### 🎉 الفعاليات الأسبوعية
- جدول `events`: بطولات، تحديات، جوائز، مواسم — بعناوين ووصف وجائزة ولون وزر لكل لغة
- تعرض التطبيق **الفعاليات النشطة فقط** (ضمن نافذة starts_at/ends_at) وتُحدّث تلقائيًا كل دقيقة
- لإضافة فعالية جديدة: صف واحد في لوحة Supabase → يظهر في التطبيق فورًا (لا تحديث مطلوب)

### تفعيل Realtime (مرة واحدة في لوحة Supabase)
`Database → Replication →Supabase Realtime` → فعّل `penalty_rooms`.
